...
class GuildMemberUpdate extends Event {
  ...
  async run(oldMember, newMember) {
    ...
    if (newMember.roles.cache.has(config.roles.verified)) {
      return;
    }

    // Make sure they have the member role now
    if (!newMember.roles.cache.has(config.roles.member)) {
      return;
    }

    const category = Object.keys(config.roles.catagories).find((key) => {
      let roles = config.roles.catagories[key];
      const output = roles.some((role) => newMember.roles.cache.has(role));
      return output;
    });
    ...
    await startVerification(
      ...
      (ticket, message) => {
        verbose(
          `Created ticket with id ${ticket.id} for ${newMember.user.tag} ${newMember.id}`
        );
      },
      (message) =>
        console.log(message || "Failed to start verification: on member join."),
      category
    );
  }
}
